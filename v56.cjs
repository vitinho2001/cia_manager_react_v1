...
fs.writeFileSync('src/pages/DashboardPage.tsx.bak56', fs.readFileSync('src/pages/DashboardPage.tsx', 'utf8'), 'utf8')
fs.writeFileSync('src/pages/DashboardPage.tsx', content, 'utf8')
console.log('OK: DashboardPage.tsx atualizado com intervalo de datas')
/*===INICIO===
...
export function DashboardPage() {
  ...
  async function load() {
    ...
    try {
      ...
      const [saleRows, allSaleRows, ingRows, purRows, compRows, recRows, recItemRows, adjRows] = await Promise.all([
        listSales(orgId, from, to).catch(() => []),
        listSales(orgId).catch(() => []),
        listIngredients(orgId).catch(() => []),
        listPurchases(orgId, undefined, to).catch(() => []),
  ...
  const trendLabel = useMemo(() => {
    ...
    const prevKey = monthKey(new Date(fromParts[0], fromParts[1] - 2, 1))
  ...
  return (
    <div className="page-container">
      <PageHeader eyebrow="Visao geral" title="Bem vindo" description="Indicadores do periodo selecionado, calculados a partir das vendas, compras e estoque." actions={<><label className="range-label">De<input type="date" className="select-control" value={from} onChange={(e) => setFrom(e.target.value)} /></label><label className="range-label">Ate<input type="date" className="select-control" value={to} onChange={(e) => setTo(e.target.value)} /></label></>} />
      <section className="stats-grid">
        <StatCard label="Faturamento" value={brl(revenue)} detail="Periodo selecionado" icon={<CircleDollarSign size={19}/>} trend={trendLabel} />
