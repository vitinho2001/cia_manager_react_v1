...
const [channelFilter, setChannelFilter] = useState<string>('all')
  ...
  const [editChannel, setEditChannel] = useState<SaleChannel>('counter')
  const [editQty, setEditQty] = useState('1')
  const [editTotal, setEditTotal] = useState('')
  useEffect(() => {
    if (editingSale) {
      setEditChannel(editingSale.channel)
      setEditQty(String(editingSale.quantity))
...
setEditTotal(String(sale.total_amount ?? 0))
  ...
  async function saveEdit() {
    ...
    setError(null)
    try {
      await updateSale(editingSale.id, { menu_item_id: editItemId, channel: editChannel, quantity: parseFloat(editQty) || 1, total_amount: parseFloat(editTotal) || 0 })
            ...
            <tbody>
              {filteredSales.length === 0 ? (
                ...
                <tr key={s.id}>
                  ...
                  <td>{s.quantity}</td>
                  <td>{brl(Number(s.total_amount) || 0)}<small style={{ display: 'block', fontSize: 11, color: '#6b7280' }}>{brl((Number(s.total_amount) || 0) / (Number(s.quantity) || 1))} /un</small></td>
      ...
      {editingSale && (
        <div className="modal-overlay">
          <div className="modal">
            ...
            <label>Canal
              <select className="select-control" value={editChannel} onChange={(e) => setEditChannel(e.target.value as SaleChannel)}>
                <option value="counter">Balcao</option>
                <option value="ifood">iFood</option>
                <option value="bysell">BySell</option>
            ...
            <label>Total (R$)
              <input type="number" min="0" step="any" value={editTotal} onChange={(e) => setEditTotal(e.target.value)} />
            </label>
            <div className="modal-footer">
